...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!apiKey) return res.status(500).json({ error: 'OpenAI API key niet geconfigureerd in Vercel' });
    ...
    // AUTOMATISCHE DETECTIE: Wordt alleen geactiveerd bij de SNOOD Expert V2 frontend
    if (imagePart && textPart && textPart.text.includes('ORIGINELE')) {
      try {
        ...
        if (detectData.choices && detectData.choices[0]) {
        ...
        console.error("Fout tijdens automatische genre/tag detectie:", err);
      }
    }
...
Sluit af met de 'Pro Insight': "${bestExpert.pro_insight}"`;
    ...
    // MAP PARTS NAAR EEN ZUIVERE OPENAI STRUCTUUR (Gecorrigeerd zonder mc/oc verwarring)
    ...
    // CORE MASTERCLASS GENERATOR AANROEP (Altijd via gpt-4o om de JSON goed te interpreteren)
